...
HttpSession session = request.getSession();
        ...
        try {
            Brands br = ac.getBrandByName(name);
            
            if(br != null){
                String error = "Brand đã tồn tại";
                request.setAttribute("error", error);
                request.getRequestDispatcher("add-brand.jsp").forward(request, response);
            }else if(name.length() == 0){
                String error = "Mời nhập lại Brand Name";
                request.setAttribute("error", error);
                request.getRequestDispatcher("add-brand.jsp").forward(request, response);
            }else {
                ac.addNewBrands(name);
                request.setAttribute("mess", "Thêm brand thành công");
                request.getRequestDispatcher("listBrands").forward(request, response);
            }
        } catch (Exception e) {
        }
    } 
